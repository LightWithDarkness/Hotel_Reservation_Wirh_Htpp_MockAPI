...
export class ReservationFormComponent {
  ...
  onSubmit() {
    ...
    let id = this.activatedRoute.snapshot.params['id'];
    if (id)
      this.rs.updateReservation({ ...newRservation, id }).subscribe(() => console.log('Reservation updated successfully'));
    else
      this.rs.addReservation(newRservation).subscribe(() => console.log('Reservation added successfully'));
    //
